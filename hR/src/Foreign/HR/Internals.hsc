...
-- The original code to import R variables does not work: the address
-- returned by peek is invalid. After much tinkering we discovered
-- that inserting 'trace ""' before each peek fixes the problem!
-- Note that trace employs unsafePerformIO, and this can (and does)
-- change the execution order...in a helpful way here.

-- The word "DELAYPEEK" was used in the early days because we
-- thought (as an imperative progammer might) that the trace
-- introduced some delay, perhaps resolving a race condition.
-- Testing showed this hypothesis to be wrong, and a more
-- likely explanation involves changes to the execution order.

-- The HaskellR team notes that there is a ghci bug that
-- requires a work-around to use data from an R DLL (see
-- inline-r/src/Lanuage/R/Globals.hs).

#let extsexp f = "\
foreign import ccall unsafe \"&R_%s\" r_%s_ptr :: Ptr R_EXP\n\
r_%s :: IO R_EXP\n\
r_%s = trace \"\" peek r_%s_ptr\n\
r%s :: IO REXP\n\
r%s = extREXP =<< r_%s\n\
", #f,#f,#f,#f,#f,#f,#f,#f ""
