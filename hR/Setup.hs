{-
  ...
  The classic autoconf and configure process fails when
  dependencies fail to provide config hooks like pkg-config.
  Here R and Haskell libs are used instead, and the only
  requirement is that the desired version of R be found in PATH.
  Under Windows MSYS Unix tools like the shell must 
  also be found in PATH (can be provided by Haskell Stack, or
  by Rtools---but be mindful that different versions of MSYS can
  conflict with each other).

  This file does what a configure file would normally do, configure
  based on architectore, OS, R version, etc. hR.bouldinfo is
  generated and placed in a build directory under .stack-work, and
  a convenience script runhr is written to ~/.local/bin/runhr. This
  script can be used to run scripts that use the hR library without
  having to specify paths to R libraries. [This design is not
  consistent with modern functional package management, but for
  small stand-alone projects this should not be a problem.]

  To build hR as a stand-alone package use 'stack install' in the
  root directory. Then the REPL can be started using
  'stack exec hr'. To run scripts that depend on hR use
  'runhr foo.hs'. By default the -fno-ghci-sandbox option is 
  used to prevent stack overflow. This disables debugging in
  ghci. To take your chances with stack overflows and enable
  debugging use 'runhr foo.hs d'.

  hR has been designed so that it is easy to add it to the HaskellR
  suite (only the Unix/Nix config has been tested).
  Simply copy the package to HaskellR, and add hR to the
  list of packages in the HaskellR stack.yaml file. [This works
  because stack --nix places the Nix version of R in PATH where
  hR's Setup.hs can find it. Setup.hs assumes it is being run
  in a Nix context when the shell environment variable
  begins with "/nix".]

  To build all of the packages in HaskellR (including hR) use
  'stack --nix install'. HaskellR's H shell can be started with
  with 'stack --nix exec H', and a REPL using hR can be started
  with 'stack --nix exec hr'. With this configuration the hR
  library can be used in a Jupyter notebook thanks to the
  IHaskell kernel provided by HaskellR.
...
nixEnvTest :: Maybe String -> Bool    
nixEnvTest  Nothing = False
nixEnvTest  (Just path) = "/nix" `isPrefixOf` path
...
confhook (g,b) conf = do
  ...
  env <- getEnvironment
  let shell = lookup "shell" env
  let nixEnv = nixEnvTest shell
    
  if nixEnv
     then putStrLn "Nix subsystem detected."
     else putStrLn "Nix subsystem not detected."     
  ...
  if nixEnv 
    then hPutStrLn handle "  stack --nix exec ghci -- $1 \\"
    else hPutStrLn handle "  stack exec ghci -- $1 \\"        
  hPutStrLn handle $ ("    -L\""++rHomeDir++binSuffix++"\"")++" -lR"
  hPutStrLn handle "else"
  hPutStrLn handle "  echo \"Debugging disabled (-fno-ghci-sandbox)\""
  if nixEnv
     then hPutStrLn handle "  stack --nix exec ghci -- -fno-ghci-sandbox $1 \\"
     else hPutStrLn handle "  stack exec ghci -- -fno-ghci-sandbox $1 \\"
  hPutStrLn handle $ ("    -L\""++rHomeDir++binSuffix++"\"")++" -lR"
  hPutStrLn handle "fi"
  hClose handle
